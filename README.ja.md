...
🎬 **デモ動画**: https://youtu.be/bzp4HG2sUcQ （3分32秒、日本語ナレーション＋英語字幕）
...
| ✅ | ✅ | ✅ | 受理：本人からの正当な確認 |
| ✅ | ❌ | ✅ | 拒否：合言葉が一致しない |
| ✅ | ✅ | ❌ | 拒否：合言葉は合っているが、登録済みの家族ではない（AI音声クローンを使う詐欺犯など、別の人間） |
| ❌ | — | — | 拒否：World ID の検証に失敗（AI だけでは、そもそも Orb 認証済みの人間として証明できない） |

合言葉は事前に共有する秘密ではなく、**親が通話のたびにその場で決めるチャレンジ**です。攻撃は2段階で止まります。**AI だけ**（自動音声の電話など）は Orb 認証済みの人間ではないので、World ID の証明そのものが作れません。**AI 音声クローンを使う詐欺犯（人間）**は、自分の World ID で証明でき、通話を聞いているので合言葉も入力できますが、登録済みの家族ではないので拒否されます。デモの2つ目の例（なりすましテスト）は、この後者を示しています。また合言葉が proof に焼き込まれるため、過去の proof を使い回すこともできません。
...
* **World ID 実統合（IDKit）による AI音声クローン対策**: `worldid/`（Express による RP署名・検証サーバー + `voice_challenge.html`）。イベント前に使っていた仮のデモページ（疑似 `nullifier_hash`）はリポジトリから削除済みで、World App 実機での proof 生成 → World v4 verify API → 合言葉（`signal_hash`）と登録済み家族（`nullifier`）の照合まで動作確認済み
...
AI（Claude）の関わり方を、ファイル単位で明記する。

* **コード（開発者本人が実装）**: circom 回路・Rust（`src/`）・Solidity（`contracts/`）・World ID の判定ロジック（`worldid/server.js` と、`voice_challenge.html` 内の IDKit 呼び出し・検証処理）は、すべて開発者本人（ソロ開発）が書いた。Claude はコーチとして、設計の相談・コードレビュー・バグの指摘・ビルド/テストの実行確認を行ったが、コードは書いていない。バグの修正も本人が行った
* **コードの例外**: イベント中に、開発者の依頼で Claude が `worldid/public/voice_challenge.html` のデモUIの一部（説明文・パネルの並び順・手順番号・証明結果の要約表示）を編集した。また `.mcp.json` の起動パスを相対パスに書き換えた。提出前の整理として、練習用のページ（`worldid/public/index.html`）と、それだけが使っていた練習用エンドポイント（`/api/verify-proof`）を削除した
* **ドキュメント（Claude が大部分を執筆・編集）**: `README.md`（英語）/ `README.ja.md`（日本語）/ `docs/`（`SPEC.md`・`PITCH.md`・`PITCH.en.md`・`SUBMISSION.en.md`・`HANDOFF.md`）/ `demo/narration_script.md` は、開発者との対話をもとに Claude が大部分を書き、内容は開発者が確認・判断している
* **デモ動画（編集を Claude が支援）**: 画面録画はすべて開発者本人が実際の環境で撮影したもの。タイトルカード・進捗バッジ・テロップの画像生成（Python/Pillow）と、ffmpeg によるカット・結合は Claude が行った。ナレーションは開発者本人の声で、AI音声は使っていない
