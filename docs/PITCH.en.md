...
(The 2026-09-22 revision still stands: Scene 3 (AI voice-clone defense) has been cut from the demo, and its time reallocated to Family Constitution. The World ID voice-clone-defense design itself isn't gone — it's just not performed live; it's covered in the Q&A prep section instead. Update 2026-09-26: the real IDKit integration was built during the event, and a World ID scene is back in the demo video (`demo/family_proof_final_en.mp4`).)
...
**Q: Is World ID actually integrated?**
A: Yes. Before the event it was a JS placeholder page (now deleted from the repo), but during the event I built a real integration with IDKit (`worldid/`). RP signing uses `@worldcoin/idkit-server`, proofs are generated in the real World App, and the server verifies them through the World v4 verify API. As an AI-voice-clone defense, the passphrase heard on the phone is bound into the proof's `signal`, and the server checks three things: an Orb-verified real human, a matching passphrase, and a registered family member's nullifier. The right passphrase from the wrong person is rejected. The limitation: "registered family member" is currently a server-side nullifier allowlist, not yet linked to the on-chain `FamilyRegistry`.
...
A: The ZK circuit, RLN, FamilyRegistry, notification infrastructure, anonymous statistics, Family Constitution — the trust-approval-protocol extension I showed today — and the MCP-server integration that lets Claude itself operate the AI Agent role via real tool calls, were all built and verified on World Chain Sepolia before the event started (through Sept 24). What I newly built during the event (Sept 25–27) is the real World ID integration with IDKit (`worldid/`, the AI voice-clone defense), replacing the earlier JS placeholder page, which has been deleted from the repo. The rest of the event time went to final rehearsal, recording the demo video, and finishing the Continuity submission writeup. This is documented explicitly in the README and the Continuity submission writeup.

**Q: Did you write all the code yourself? How did you use AI?**
A: I wrote all the Solidity and Rust code myself. Claude acted only as a coach — design review, confirming builds/tests. It found and pointed out implementation bugs, but I made every fix myself. I also wrote the World ID verification logic (`server.js`, and the page's IDKit call and verification flow). One exception: at my request, Claude edited part of the World ID demo page (`voice_challenge.html`) — explanatory text, layout order, and display. Outside the code, Claude wrote most of the documentation such as the README, which I reviewed. For the demo video, I recorded all the screen footage myself, and Claude helped make the cards and captions and edit it together. The narration is my own voice. The README lists, file by file, where AI was involved.
