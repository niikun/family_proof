...
**2026-09-22改訂: シーン3（AI音声クローン対策）はデモから削除し、Family Constitutionに時間を厚く配分**（ユーザー判断: 「デモが長すぎて分かりづらい、Family Constitutionの方が面白い」）。World ID音声クローン対策の設計自体は無くなっていない——デモで実演しないだけで、Q&A想定問答に説明を残してある。

| パート | 時間 | 内容 |
|---|---|---|
| 0. フック（問題提起 + 再定義） | 0:00–0:25 | オレオレ詐欺の矛盾 → 「実はもっと大きい話」への転換 |
| 1. シーン1: 家族なりすまし確認 | 0:25–0:45 | secretを出さずに検証成功 |
| 2. シーン2: RLN 使い回し検知 | 0:45–1:25 | 盗まれたsecretが2回使うと復元・失効される |
| 3. Family Constitution（本編） | 1:25–3:30 | Claudeが実際にAI Agent役 → 提案 → 攻撃者/Claude自身が失敗 → 隣人2人が承認で実行 |
...
## 1. シーン1: 家族なりすまし確認（0:25–0:45、~20秒）
...
## 2. シーン2: RLN 使い回し検知（0:45–1:25、~40秒）
